...
(function(window, undefined) {
	...
	var Etw = {
		...
		skin: null,					// 窗口默认皮肤
		icon: null					// 窗口提示标签：普通提示-INFO、错误提示-ERROR、惊喜提示-LOVE 没有图标-NONE
	}
	...
	Etw.Controller = {
							...
							'<h3 class="flb" id="'+Etw.type+'_bar">' + 
								...
								'<span>' + 
									'<div onclick="javascript:void(0);" id="closeIcon_'+ Etw.type + 
										'" class="flbc sxbbs_udf_drag_close_'+Etw.type+'"></div>' + 
								'</span>' + 
							'</h3>' + 
						'</div>';
					}
		...
		setEtwPositionVal: function(type) {
			...
			if(EtmDragWin.length > 0 && EtmDragWin.is(":visible")) {
				...
				if(type == Etw.wnty.prompt_apptip) {
					EtmDragWin.css({
